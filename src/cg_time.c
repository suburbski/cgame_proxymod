...
static cvarTable_t timer_cvars[] = { { &timer_draw, "mdd_hud_timer_draw", "0", CVAR_ARCHIVE },
...
void draw_time(void)
{
  ...
  if (!timer_draw.integer) return;

  sscanf(
    timer_outline_rgba.string,
    "%f %f %f %f",
    &outline_color[0],
    &outline_color[1],
    &outline_color[2],
  ...
  sscanf(timer_item_rgba.string, "%f %f %f %f", &item_color[0], &item_color[1], &item_color[2], &item_color[3]);
  ...
  for (int i = 0; i < MAX_NADES; i++)
  {
    ...
    {
      float progress = 1.0 - (float)(nades[i].explode_time - snap->serverTime) / NADE_EXPLODE_TIME;
...
static void init_timer_cvars(void)
{
  for (int i = 0; i < ARRAY_LEN(timer_cvars); i++)
    g_syscall(
      CG_CVAR_REGISTER,
      timer_cvars[i].vmCvar,
      timer_cvars[i].cvarName,
      timer_cvars[i].defaultString,
      timer_cvars[i].cvarFlags);
}

static void update_timer_cvars(void)
{
  for (int i = 0; i < ARRAY_LEN(timer_cvars); i++) g_syscall(CG_CVAR_UPDATE, timer_cvars[i].vmCvar);
}
